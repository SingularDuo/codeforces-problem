...
Date: 2026-08-08 11:30:50
Problem name: C. AND Graph
Problem URL: https://codeforces.com/problemset/problem/986/C
Idea: 
Tag used: 
Summary: 
*/
...
constexpr int MOD = 1000000007, MOD2 = 998244353, MAXN = 5000000, LOG = 20, BLOCK = 450;
...
_______TOISECOGIAIVOI_______
{
    ...
    //     for(int sub = rmask; sub > 0; sub = (sub - 1) & rmask) if(ap[sub]) join(mask, sub);
    //     if (ap[0]) join(mask, 0);
    // }  
    // set<int> comp;
    // for(int i = 1; i <= m; i++) comp.insert(fs(a[i]));
    // cout<<comp.size();
    int res = 0;
    for(int i =1 ; i <= m; i++)
    {
        if(!vis[a[i]])
        {
            ...
            while(!q.empty())
            {
                ...
                {
                    int rmask = (~u) & ((1 << n) - 1);
                    if(vis[rmask]) goto nxt;
                    q.push(rmask); vis[rmask] = true;
                }
                nxt:
                for(int j = 0; j < n; j++)
                {
