...
 
    int find_last(int id, int l, int r, int u, int v, int val)
    {
        if (v < l || r < u || st[id] >= val)
        {
            return -1;
        }
        if (l == r)
        ...
        int mid = (l + r) >> 1;
        int res = find_last(id << 1 | 1, mid + 1, r, u, v, val);
        if (res != -1)
        {
            return res;
        }
        return find_last(id << 1, l, mid, u, v, val);
    }
} tree;
 
...
 
    for (int i = 1; i <= n; i++)
    {
        int idx = tree.find_last(1, 1, n, i + 1, n, a[i]);
        if (idx == -1)
        {
            cout << -1 << " ";
        }
