...
 
    void update(int pos, int val, int id = 1, int l = 1, int r = -1)
    {
        if (r == -1) r = n;
        if (l == r) {
            st[id] = max(st[id], val);
...
 
    int query(int ql, int qr, int id = 1, int l = 1, int r = -1)
    {
        if (r == -1) r = n;
        if (ql > qr || r < ql || l > qr) return -OO;
...
void solve()
{
    input();
    vi coord;
    for (int i = 0; i <= n; i++) coord.pb(f[i]);
    sort(nall(coord));
    coord.erase(unique(nall(coord)), coord.end());
 
    auto getID = [&](int x) {
        return lower_bound(nall(coord), x) - coord.begin() + 1;
    };
    int sz = coord.size();
    SegmentTree less, great;
    less.init(sz);
    great.init(sz);
    vi maxEqual(sz + 5, -OO);
    vi dp(n + 1, -OO);
    dp[0] = 0;
    int id0 = getID(0);
    less.update(id0, dp[0] - 0);
    maxEqual[id0] = dp[0];
    great.update(id0, dp[0] + 0);
    FOR(i, 1, n)
    {
        int currID = getID(f[i]);
        int q1 = less.query(1, currID - 1);
        if (q1 != -OO) maximize(dp[i], q1 + i);
        if (maxEqual[currID] != -OO) maximize(dp[i], maxEqual[currID]);
        int q3 = great.query(currID + 1, sz);
        if (q3 != -OO) maximize(dp[i], q3 - i);
        less.update(currID, dp[i] - i);
        maximize(maxEqual[currID], dp[i]);
        great.update(currID, dp[i] + i);
    }
    cout<<dp[n]<<endl;
}
 
