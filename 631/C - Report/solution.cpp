...
Date: 2026-07-07 17:48:20
Problem name: C. Report
Problem URL: https://codeforces.com/problemset/problem/631/C
Idea: 
xét thao tác ở j và i (j > i) nếu rj >= ri && lj < ri thì xóa luôn i
sau khi loại bỏ, ta sẽ được một dãy thao tác giảm dần nghiêm ngặt theo r
vậy thì phần tử từ r1 -> n luôn giữ nguyên
tổng quát: khi chuyển từ thao tác i sang thao tác i + 1 (ri -> ri+1) thì khoảng từ ri+1 -> ri 
không bao giờ bị thay đổi nữa
 
Tag used: 
Summary: 
*/
...
_______TOISECOGIAIVOI_______
{
    ...
    for(int _ = 1; _ <= m; _++)
    {
        int t = qs[_].fi, r = qs[_].se;
        while(!st.empty() && st.top().se <= r) st.pop();
        st.push({t,r});
    }
    st.push({0,0});
    vector<pii> os;
    ...
    int r1 = os[0].se;
    for (int i = r1 + 1; i <= n; i++) res[i] = a[i];
    vi sorted(r1);
    for (int i = 0; i < r1; i++) sorted[i] = a[i + 1];
    sort(nall(sorted));
    int L = 0, R = r1 - 1, currPos = r1; 
    for (int i = 0; i < os.size() - 1; i++) 
    {
        ...
        while (currPos > nxtR) 
        {
            if (currT == 1) 
            { 
                res[currPos] = sorted[R];
                R--;
            } 
            else 
            { 
                res[currPos] = sorted[L];
                L++;
            }
            currPos--;
        }
