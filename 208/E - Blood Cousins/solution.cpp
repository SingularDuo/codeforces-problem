...
 
bai toan dua ve dang:
voi moi truy van p,v, goi P la to tien thu p cua v
dem so luong dinh nam trong cay con goc P co depth = depth[v]
moi dinh se luu 1 map
neu cai dat on thi tong size cua cac map chi khoang N
(n - 1)/ 1 + 1
dsu on tree
 
*/
...
int n, m;
vi g[MAXN + 5];
int up[MAXN + 5][LOG + 5], depth[MAXN + 5];
map<int, int> freq[MAXN + 5];
vector<pair<int, int>> qAnc[MAXN + 5];
int ans[MAXN + 5];
...
void dfs(int u)
{
    ...
    for(int v : g[u])
    {
        ...
        dfs(v);
        if(freq[v].size() > freq[u].size()) swap(freq[u], freq[v]);
        for(auto &x : freq[v]) freq[u][x.fi] += x.se;
    }
    for(auto &q : qAnc[u]) 
    {
        int target_depth = q.first;
        int q_idx = q.second;
        if (freq[u].count(target_depth)) ans[q_idx] = max(0LL, freq[u][target_depth] - 1);
        else ans[q_idx] = 0;
    }
}
 
                            ...
                            _______TOISECOGIAIVOI_______
{
    ...
    {
        int v, p; cin>>v>>p;
        int anc = kthAncestor(v, p);
        if(anc == 0) ans[i] = 0;
        else qAnc[anc].pb({depth[v], i});
    }
    for(int u : root) dfs(u);
