...
struct Query 
{
    ...
    {
        int ab = l / BLOCK, bb = other.l / BLOCK;
        if(ab != bb) return ab < bb;
        return (ab % 1) ? (r < other.r) : (r > other.r);
    }
};
int n, m;
vi g[MAXN + 5];
int a[MAXN + 5], c[MAXN + 5];
int tin[MAXN + 5], tout[MAXN + 5], euler[MAXN + 5], timer = 0;
int res[MAXN + 5];
int freq[MAXN + 5];      
int sumFreq[MAXN + 5];
vector<Query> qs;
