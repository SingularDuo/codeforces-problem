...
Date: 2026-08-06 18:20:38
Problem name: G. Multidimensional Queries
Problem URL: https://codeforces.com/contest/1093/problem/G?fbclid=IwY2xjawThSv1wZG9mBWV4dG4DYWVtAjEwAGJyaWQRMTZNeUlORjdkUTRJMVJjUHNzcnRjBmFwcF9pZBAyMjIwMzkxNzg4MjAwODkyAAEeqi67R7KUncbKJ4cuBrTXqYq8jjiBrflb7B8gJlS2jKhT5wARt_gQgEBnAiA_aem_W3rdnQylWZS-oTfZBiJemw
Idea: 
Tag used: 
Summary: 
*/
...
Node createNode(Point p)
{
    ...
    for(int mask = 0; mask < maxMask; mask++)
    {
        ...
        for(int i = 1; i <= k; i++)
        {
            if(getbit(mask, i - 1)) val += p.dimen[i];
            else val -= p.dimen[i];
        }
        res.mx[mask] = val;
        res.mn[mask] = val;
...
Node MERGE(Node left, Node right)
{
    ...
    for(int mask = 0; mask < maxMask; mask++)
    {
        res.mx[mask] = max(left.mx[mask], right.mx[mask]);
        res.mn[mask] = min(left.mn[mask], right.mn[mask]);
    }
    return res;
}
...
_______TOISECOGIAIVOI_______
{
    ...
    cin>>n>>k;
    maxMask = (1 << k);
    for(int i = 1; i <= n; i++) for(int j = 1; j <= k; j++) cin>>point[i].dimen[j];
    build(1,1,n);
    int q; cin>>q;
    while(q--)
    {
        ...
        {
            int x; cin>>x;
            for(int i = 1; i <= k; i++) cin>>point[x].dimen[i];
            update(1,1,n,x);
        }
        else 
        {
            ...
            for(int mask = 0; mask < maxMask; mask++) maximize(maxDist, res.mx[mask] - res.mn[mask]);
            cout<<maxDist<<endl;
        }
    } 
    KILL();
