...
void solve() {
    ...
    MinPrefix[0] = a[0];
    for (int i = 1; i < n; i++) MinPrefix[i] = min(MinPrefix[i - 1], a[i]);
 
    vector<int> MaxSuffix(n);
    MaxSuffix[n - 1] = a[n - 1];
    for (int i = n - 2; i >= 0; i--) MaxSuffix[i] = max(MaxSuffix[i + 1], a[i]);
    
    bool ValidTree = true;
    for (int i = 0; i < n - 1; i++) {        
        if (MinPrefix[i] > MaxSuffix[i + 1]) {
            ValidTree = false;
            break;
        }
    }
    if (ValidTree) cout << "Yes
";
    else cout << "no
";
}
