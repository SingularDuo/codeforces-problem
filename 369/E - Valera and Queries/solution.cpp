...
_______TOISECOGIAIVOI_______
{
    ...
    for(int i = 1; i <= m; i++) 
    {
        ...
        res[i] = n; 
        if (p[0] > 1) qs[p[0] - 1].pb({1, p[0] - 1, i});
        for(int j = 0; j < x - 1; j++) 
        {
            if (p[j] + 1 <= p[j+1] - 1)  qs[p[j+1] - 1].pb({p[j] + 1, p[j+1] - 1, i});
        }
        if (p[x - 1] < lim) qs[lim].pb({p[x - 1] + 1, lim, i});
    }
    for(int r = 1; r <= lim; r++)
    {
        for(auto &l : group[r]) update(1, 1, lim, l, l, 1);
        for(auto &_ : qs[r])
        {
            int ngu = get(1, 1, lim, _.L, lim);
            res[_.id] -= ngu;
        }
    }
