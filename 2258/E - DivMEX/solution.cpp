...
void sieve()
{
    FOR(i, 2, MAXN) if(spf[i] == 0)
    {
        spf[i] = i;
        for(int j = i * i; j <= MAXN; j += i) if(spf[j] == 0) spf[j] = i;
    }
    FOR(i, 2, MAXN)
    {
        int tmp = i;
        while(tmp % spf[i] == 0) tmp /= spf[i];
        if(tmp == 1)
        {
            pw.pb(i);
            idP[i] = pw.size();
...
void solve()
{
    ...
    FOR(i, 1, n) 
    {
        ...
        while (tmp > 1) 
        {
            ...
            {
                pk *= p;
                update(1, 1, m, idP[pk], i);
                tmp /= p;
            }
        }
        EACH(curr, qs[i]) 
        {
            int id = curr.fi, minL = curr.se;
            if (id == 1 || get(1, 1, m, 1, id - 1) >= minL) {
                res.pb(id);
            }
        }
    }
    sort(nall(res));
    res.erase(unique(nall(res)), res.end());
    cout << res.size() << endl;
    if (!res.empty()) {
        EACH(x, res) cout << pw[x - 1] << ' ';
        cout << endl;
    }
 
    EACH(id, ids) {
