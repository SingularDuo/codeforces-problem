...
_______TOISECOGIAIVOI_______
{
    ...
    {
        int j = i;
        while(j <= m && eds[j].w == eds[i].w) j++;
        for(int _ = i; _ < j; _++) if(fs(eds[_].u) != fs(eds[_].v)) inMST[_] = true; 
        for(int _ = i; _ < j; _++) join(eds[_].u, eds[_].v);
        i = j; 
    }
    ...
    for(int i = 1; i <= m; ) 
    {
        ...
        while(j <= m && eds[j].w == eds[i].w) j++;
        int total = 0;
        for(int _ = i; _ < j; _++) total += inMST[_];
        res += (total - cnt[eds[i].w]);
        i = j;
    }
    cout<<res;
    TIMER(start);
