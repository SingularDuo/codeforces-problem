...
constexpr int MOD = 1000000007, MOD2 = 998244353, MAXN = 1000, LOG = 20, BLOCK = 450;
...
void print(int type) 
{
    ...
    while (i > 1 || j > 1) 
    {
        ...
        else 
        {
            if (type == 2) 
            {
                if (dp2[i][j] == dp2[i - 1][j] + cnt(grid[i][j], 2)) 
                {
                    path += 'D';
                    i--;
                } 
                ...
                if (dp5[i][j] == dp5[i - 1][j] + cnt(grid[i][j], 5)) 
                {
                    path += 'D';
                    i--;
                } 
...
 
    ...
    dp2[1][1] = cnt(grid[1][1], 2);
    dp5[1][1] = cnt(grid[1][1], 5);
    FOR(i, 1, n) FOR(j, 1, n) 
    {
        if (i == 1 && j == 1) continue;
        int c2 = cnt(grid[i][j], 2);
        int c5 = cnt(grid[i][j], 5);
        if (i > 1) 
        {
            minimize(dp2[i][j], dp2[i - 1][j] + c2);
            minimize(dp5[i][j], dp5[i - 1][j] + c5);
        }
        if (j > 1) 
        {
            minimize(dp2[i][j], dp2[i][j - 1] + c2);
            minimize(dp5[i][j], dp5[i][j - 1] + c5);
        }
    }
    int rea = min(dp2[n][n], dp5[n][n]);
    if (zr != -1 && rea > 1) 
    {
        cout << 1 << endl;
        FOR(i, 1, zr - 1) cout << 'D';
