...
";
    cerr << "Memory used : " << fixed << setprecision(2) << max(0.0, dynamic_mb) << " MB
";
    cerr << "===============================
";
}
...
int n, m;
vpii g[MAXN + 5];
vector<Edge> g2[MAXN + 5];
int low[MAXN + 5], num[MAXN + 5], timer = 0;
bool joint[MAXN + 5], bridge[MAXN + 5];
bitset<MAXN + 5> vis;
pii direct[MAXN + 5];
void dfs(int u, int pid)
{
    ...
    for(auto i : g[u]) if(i.se != pid)
    {
        ...
        else  
        {
            ...
            if(low[v] > num[u]) bridge[eid] = true;
            if(pid != -1 && low[v] >= num[u]) joint[u] = true;
        }
    }
    if(pid == -1 && child >= 2) joint[u] = true;
}
void dfsDirect(int u, int pid)
{
    vis[u] = true;
    for(auto &ed : g2[u]) 
    {
        int v = ed.to, eid = ed.id;
        if(eid == pid) continue;
        if(vis[v]) 
        {
            if(num[v] < num[u]) direct[eid] = {u, v};
        }
        else
        {
            direct[eid] = {u, v};
