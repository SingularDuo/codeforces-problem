...
constexpr int MOD = 1000000007, MOD2 = 998244353, MAXN = 20000000, LOG = 20, BLOCK = 450;
constexpr int ALPHA = 26, BIT = 30, MAXV = 20000000;
...
void sieve()
{
    FOR(i, 1, MAXV) spf[i] = i;
    for (int i = 2; i * i <= MAXV; i++) if (spf[i] == i) for (int j = i * i; j <= MAXV; j += i) if (spf[j] == j) spf[j] = i;
}
vi div(int x) 
{
    vi res;
