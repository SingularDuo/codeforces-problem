...
_______TOISECOGIAIVOI_______
{
    ...
    cin>>n>>s>>t;
    int cntZ1 = 0, cntO1 = 0, cntZ2 = 0, cntO2 = 0;
    for(char c : s)
    {
        if(c == '0') cntZ1++;
        else cntO1++;
    ...
    if(cntZ1 != cntZ2 || cntO1 != cntO2)
    {
        cout<<-1;
        KILL();
    }
    int currDiff = 0, maxDiff = 0, minDiff = 0;
    int res = 0;
    for (int i = 0; i < n; i++) 
    {
        if (s[i] == '0' && t[i] == '1') currDiff++;
        else if (s[i] == '1' && t[i] == '0') currDiff--;
        maximize(maxDiff, currDiff); minimize(minDiff, currDiff);
        res = max(res, maxDiff - minDiff); 
    }
    cout<<res;
    KILL();
}