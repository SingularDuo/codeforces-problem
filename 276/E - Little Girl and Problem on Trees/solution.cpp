...
int n, q;
int belong[MAXN + 5], posChain[MAXN + 5];
vi g[MAXN + 5];
vi chain[MAXN + 5];
vector<vi> st;
vector<vi> lazy;
void dfs(int u, int par, int chainID)
{
    chain[chainID].pb(u);
    belong[u] = chainID;
    posChain[u] = chain[chainID].size() - 1;
    EACH(v, g[u]) if(v != par) dfs(v, u, chainID);
}
void push(int treeID, int id)
{
    if(lazy[treeID][id] != 0)
    {
        int &val = lazy[treeID][id];
        lazy[treeID][id * 2] += val;
        lazy[treeID][id * 2 + 1] += val;
        st[treeID][id * 2] += val;
        st[treeID][id * 2 + 1] += val;
        val = 0;
    }
}
void update(int treeID, int id, int l, int r, int ql, int qr, int val)
{
    if(l > qr || r < ql) return;
    if(l >= ql && r <= qr)
    {
    ...
    push(treeID, id);
    int mid = (l + r)  / 2;
    update(treeID, id * 2, l, mid, ql, qr, val);
    update(treeID, id * 2 + 1, mid + 1, r, ql, qr, val);
    st[treeID][id] = st[treeID][id * 2] + st[treeID][id * 2 + 1];
}
int get(int treeID, int id, int l, int r, int ql, int qr)
{
    if(l > qr || r < ql) return 0;
    if(l >= ql && r <= qr)  return st[treeID][id];
    push(treeID, id);
    int mid = (l + r) / 2;
    return get(treeID, id * 2, l, mid, ql, qr) + get(treeID, id * 2 + 1, mid + 1, r, ql, qr);
}
 
 
_______TOISECOGIAIVOI_______
{
    ...
    int cnt = 0;
    for(int v : g[1]) dfs(v, 1, ++cnt);
    st.resize(n + 1); lazy.resize(n + 1);
    st[0].resize(4 * n + 5, 0);
    lazy[0].resize(4 * n + 5, 0);
    for(int i = 1; i <= cnt; i++) 
    {
        st[i].resize(4 * (int)(chain[i].size()) + 5, 0);
        lazy[i].resize(4 * (int)(chain[i].size()) + 5, 0);
    }
    int rootVal = 0; 
    while(q--)
    {
        ...
        if(t == 0)
        {
            ...
            {
                rootVal += x;
                if(d > 0) update(0, 1, 1, n, 1, min(n, d), x);
            }
            else
            {
                int useChain = belong[v];
                int place = posChain[v]; 
                int sz = chain[useChain].size();
                update(useChain, 1, 0, sz - 1, max(0LL, place - d), min(sz - 1, place + d), x);
                if(d >= place + 1)
                {
                    int rem = d - (place + 1);
                    rootVal += x; 
                    if(rem > 0)
                    {
                        update(0, 1, 1, n, 1, min(n, rem), x);
                        int maxSub = min(sz - 1, rem - 1);
                        if(maxSub >= 0) update(useChain, 1, 0, sz - 1, 0, maxSub, -x);
                    }
                }
            }
        }
        else  
        {
            ...
            else
            {
                ...
                int depth = place + 1;
                int sz = chain[useChain].size();
                int ans = get(useChain, 1, 0, sz - 1, place, place) + get(0, 1, 1, n, depth, depth);
                cout << ans << endl;
            }
        }
    }
