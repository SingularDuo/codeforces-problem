...
void dfs(int u, int p)
{
    ...
    for(int v : g[u])
    {
        if(v == p) continue;
        dfs(v, u);
        sum0 += max(dp[v][0] + abs(lim[v].fi - lim[u].fi), dp[v][1] + abs(lim[v].se - lim[u].fi));
        sum1 += max(dp[v][0] + abs(lim[v].fi - lim[u].se), dp[v][1] + abs(lim[v].se - lim[u].se));
    }
    dp[u][0] += sum0;
    dp[u][1] += sum1;
}
void solve()
{
    ...
    {
        int x, y; cin>>x>>y;
        lim[i].fi = min(x, y); lim[i].se = max(x, y);
    }
    for(int i = 1; i <= n - 1; i++)
    {
        int u, v; cin>>u>>v;
                            ...
                            _______TOISECOGIAIVOI_______
{
    ...
    /*
    nhận xét: với mỗi đỉnh, cách chọn tối ưu nhất cahwcs chắn là l hoặc r, các phần tử ở giữa không thể tối ưu bằng
    */
    // log_resource();
    KILL();
}