...
void solve()
{
    ...
    sort(nall(sorted));
    f[0] = sorted[0];
    for(int i = 1; i <= n - 1; i++) f[i] = f[i - 1] + sorted[i];
    // để số i thì tất cả số từ 0 đến i - 1 phải xuất hiện và các số còn lại nằm trong khoảng i + 1
    // đến INF
    /*
    Muốn một số có thể thành mex của mảng thì phải có ít nhất i số có thể tạo thành 
    các số từ 0 -> i - 1
    khi sort lại và xét tới số i, muốn dc thì phải có i số trong sorted thỏa:
    sorted[i] <= i - 1 
    còn nếu có nhiều số i đã xuất hiện trong mảng thì có thể dễ dàng xử lí = việc +1
    sau khi phân loại, chắc chắn sẽ không cần quan tâm đến các phần tử có giá trị > fail
    */
    vi op(n + 5, 0);
    int fail = n;
    for(int i = 0; i <= n - 1; i++)
