...
int LCA(int u, int v)
{
    if(depth[u] != depth[v])
    {
        ...
        FORD(i, 0, 20) if(depth[up[u][i]] >= depth[v]) u = up[u][i];
    }
    if(u == v) return u;
    FORD(i, 0, 20) if(up[u][i] != up[v][i]) 
    {
        u = up[u][i];
        v = up[v][i];
    }
...
void bfsMultiSource()
{
    fill(distGlobal, distGlobal + n + 1, oo);
    queue<int> q;
    ...
    {
        int u = q.front(); q.pop();
        for(int v : g[u]) if(distGlobal[v] > distGlobal[u] + 1)
        {
            distGlobal[v] = distGlobal[u] + 1;
            q.push(v);
        }
    }
}
...
_______TOISECOGIAIVOI_______
{
    ...
    FOR(i, 1, m) 
    {
        ...
        {
            // sqrt(n)
            int res = distGlobal[node];
            for (int x : recentRed) minimize(res, dist(node, x));
            cout << res << endl;
        }
        // n * sqrt(n)
        if (recentRed.size() >= BLOCK) 
        {
            bfsMultiSource();
            recentRed.clear();
        }
