...
void solve()
{
    ...
    {
        if(cnt[i] == 0) continue;
        if(maximize(mx, cnt[i])) cntFreq = 1;
        else if(cnt[i] == mx) cntFreq++;
    }
    int res = OO;
    X = n - mx * cntFreq;
    debug(mx);
    ...
    cerr<<endl;
    int khe = mx - 1;
    // se build ra cac khoang gom cac so xuat hien mx lan, abcd....abcd....abcd
    // nhan xet duocc la cac so khac xuat hien < mx lan thi chac chan se co cach dat sao cho khoang cach >= khoang cach cua a - a
    int dist = X / khe;
    cout<<dist + cntFreq - 1<<endl;
 
}
