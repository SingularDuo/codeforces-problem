...
bool isPal(int l, int r, StringHash &hs, StringHash &hr) 
{
    return hs.getHash(l, r) == hr.getHash(getIdx(r), getIdx(l));
}
 
_______TOISECOGIAIVOI_______
{
    ...
    for(int len = 2; len <= n; len++)
    {
        ...
        {
            int r = l + len - 1;
            if(!isPal(l, r, hs, hr))  continue;
            int half = len / 2;
            dp[l][r] = 1;
            if(half) dp[l][r] = dp[l][l + half - 1] + 1;
            res[dp[l][r]]++;
        }
    }
