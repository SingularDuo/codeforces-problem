...
constexpr int MOD = 1000000007, MOD2 = 998244353, MAXN = 500000, LOG = 20, BLOCK = 450;
...
char a[MAXN + 5];
vi g[MAXN + 5];
int p[MAXN + 5], depth[MAXN + 5];
int tin[MAXN + 5], tout[MAXN + 5], timer = 0, euler[MAXN + 5];
vi nodeDepth[MAXN + 5], prefixXor[MAXN + 5];
void dfs(int u)
{
    tin[u] = ++timer;
    euler[timer] = u;
...
_______TOISECOGIAIVOI_______
{
    ...
    depth[1] = 1;
    dfs(1);
    for(int i = 1; i <= n; i++) nodeDepth[depth[i]].pb(i);
    for(int i = 1; i <= n; i++) if(!nodeDepth[i].empty()) 
    {
        sort(nall(nodeDepth[i]), cmp);
        int sz = nodeDepth[i].size();
        prefixXor[i].resize(sz + 1, 0);
        for(int j = 0; j < sz; j++)  prefixXor[i][j + 1] = prefixXor[i][j] ^ toMask(a[nodeDepth[i][j]]);
    }
    while(m--)
    {
        int u, h; cin >> u >> h;
        if (h < depth[u] || nodeDepth[h].empty()) 
        {
            cout << "Yes"<<endl;
            continue;
        }
        auto it1 = lower_bound(nodeDepth[h].begin(), nodeDepth[h].end(), u, [](int node, int val) {
            return tin[node] < tin[val];
        });
        auto it2 = upper_bound(nodeDepth[h].begin(), nodeDepth[h].end(), u, [](int val, int node) {
            return tout[val] < tin[node];
        });
 
        int L = distance(nodeDepth[h].begin(), it1);
        int R = distance(nodeDepth[h].begin(), it2) - 1;
        if (L > R) 
        {
            cout << "Yes"<<endl;
        } 
        ...
        {
            int mask = prefixXor[h][R + 1] ^ prefixXor[h][L];
            if ((mask & (mask - 1)) == 0) cout << "Yes"<<endl;
            else cout << "No"<<endl;
        }
    }
 
