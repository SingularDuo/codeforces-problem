...
struct Trie 
{
    ...
    void insert(const string &s)
    {
        ...
        {
            int id = c - 'a';
            if(!ch[curNode][id])ch[curNode][id] = ID++;
            curNode = ch[curNode][id];
            pre[curNode]++;
        }
        cnt[curNode]++;
    }
    ...
    int prefixCount(const string &s)
    {
        ...
        {
            int idx = c - 'a';
            if (!ch[curNode][idx]) return 0;
            curNode = ch[curNode][idx];
        }
        return pre[curNode];
    }
    void initDP()
    ...
    void dfs(int id)
    {
        ...
        for (int i = 0; i < 26; i++) 
        {
            if (ch[id][i] != 0)
            {
                ...
                child++;
                dfs(v);
                if (!win[v]) win[id] = true;
                if (!lose[v]) lose[id] = true;
            }
        }
        if (child == 0)
        {
...
/*
Người chơi đầu tiên luôn thắng khi:
- Tồn tại một prefix độ dài lẻ và không thể rẽ nhánh thành độ dài chẵn (1)
-  nếu không có (1), tồn tại một prefĩx chẵn có thể rẽ lại thành lẻ vào bước lẻ
còn lại luôn thua
*/
 
