...
Date: 2026-07-28 19:23:17
Problem name: E. Tree Master
Problem URL: https://codeforces.com/problemset/problem/1806/E
Idea: 
Tag used:
Summary: 
*/
...
constexpr int MOD = 1000000007, MOD2 = 998244353, MAXN = 100000, LOG = 20, BLOCK = 320;
...
 
int n, q;
int a[MAXN + 5], p[MAXN + 5], depth[MAXN + 5];
int idx[MAXN + 5];
vi depthNode[MAXN + 5];
vector<int> memo[MAXN + 5];
 
void prepare()
{
    depth[1] = 1;
    ...
    {
        depth[i] = depth[p[i]] + 1;
        idx[i] = (int)depthNode[depth[i]].size();
        depthNode[depth[i]].pb(i);
    }
    for (int d = 1; d <= n; d++)
    {
        if (depthNode[d].empty()) break;
        if ((int)depthNode[d].size() <= BLOCK)
        {
            for (int u : depthNode[d]) memo[u].assign(idx[u] + 1, -1);
        }
    }
}
 
int getF(int x, int y) 
{
    ...
    if ((int)depthNode[d].size() <= BLOCK)
    {
        if (memo[x][idx[y]] != -1)
            return memo[x][idx[y]];
        int res = a[x] * a[y] + getF(p[x], p[y]);
        return memo[x][idx[y]] = res;
    }
    return a[x] * a[y] + getF(p[x], p[y]);
}
 
