...
Date: 2026-08-04 21:35:38
Problem name: B. Evanescent
Problem URL: https://codeforces.com/contest/2254/problem/B
Idea: 
Tag used: 
Summary: 
*/
...
void solve()
{
    cin>>n>>s;
    int iniR = 0;
    for(int i = 0; i <= n - 2; i++) iniR += (s[i] != s[i + 1]);
    iniR++;
    int res = iniR;
    // debug(res);
    for(int i = 1; i <= n - 2; i++)
    {
        ...
        {
            if(s[i - 1] == s[i + 1]) minimize(res, iniR - 2);
            else minimize(res, iniR - 1);
        }
    }
    cout<<res<<endl;
}
