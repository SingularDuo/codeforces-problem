...
Date: 2026-08-04 22:02:45
Problem name: D. Silhouette
Problem URL: https://codeforces.com/contest/2254/problem/D
Idea: 
Tag used: 
Summary: 
*/
...
int n;
int a[MAXN + 5], val[MAXN + 5], orgB[MAXN + 5], b[MAXN + 5];
void solve()
{
    cin >> n;
    for (int i = 1; i <= n; i++)
...
 
    ...
    if (k == 1) val[1] = 1;
    else val[k] = val[k - 1] + 1;
    for (int i = 1; i <= n; i++)
    {
        int id = lower_bound(a + 1, a + k + 1, orgB[i]) - a;
        cout << val[id] << " ";
    }
    cout << endl;
}
