...
 
    const int MAXX = 512;
    static int mini[MAXX];
    for (int x = 0; x < MAXX; x++) mini[x] = oo;
 
    // subsequence rỗng: XOR = 0, giá trị kết thúc = -1
    mini[0] = -1;
 
    for (int i = 1; i <= n; i++) {
        static int old[MAXX];
        for (int x = 0; x < MAXX; x++) old[x] = mini[x];
 
        for (int x = 0; x < MAXX; x++) {
            if (old[x] < a[i]) {
                int nx = x ^ a[i];
                mini[nx] = min(mini[nx], a[i]);
            }
        }
    }
 
