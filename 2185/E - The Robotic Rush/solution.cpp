...
void solve()
{
    ...
    cin >> s;
    sort(b.begin(), b.end());
    vector<pair<ll,int>> distR, distL; 
 
    for (int i = 0; i < n; i++)
    {
        ll x = a[i];
        auto it1 = upper_bound(b.begin(), b.end(), x);
        if (it1 != b.end()) distR.pb({*it1 - x, i});
        else distR.pb({OO, i}); 
 
        auto it2 = lower_bound(b.begin(), b.end(), x);
        if (it2 != b.begin()) distL.pb({x - *prev(it2), i});
        else distL.pb({OO, i}); 
    }
    sort(distR.begin(), distR.end());
    sort(distL.begin(), distL.end());
    vector<int> cut(n, 0);
    ll curD = 0, maxD = 0, minD = 0;
    int ptR = 0, ptL = 0;
    int cnt = 0;
    for (int i = 0; i < k; i++)
    {
