...
long long extendedGcd(long long a, long long b, long long &x, long long &y)
{
    ...
    long long x1, y1;
    long long d = extendedGcd(b % a, a, x1, y1);
    x = y1 - (b / a) * x1;
    y = x1;
    return d;
}
...
 
    ...
    long long inv_n = modinv(n, MOD9);
    long long inv_n2 = (inv_n * inv_n) % MOD9;
    for (int i = 1; i <= n; i++)
    {
        long long sum_cnt_y = 0;
        int k_i = a[i].size();
        for (int item : a[i])
        {
            sum_cnt_y = (sum_cnt_y + cnt_item[item]) % MOD9;
        }
 
        long long term = (sum_cnt_y * modinv(k_i, MOD9)) % MOD9;
 
        total_prob = (total_prob + term) % MOD9;
    }
    long long final_ans = (total_prob * inv_n2) % MOD9;
    cout << final_ans << endl;
 
    // === END SOLUTION HERE ===
 
