...
Date: 2026-08-17 22:23:26
Problem name: E. Busy Beaver
Problem URL: https://codeforces.com/contest/2257/problem/E
Idea: 
Tag used: 
Summary: 
*/
...
void solve()
{
    int n, x; cin>>n>>x;
    vector<vi> a(n), d(n);
    vector<vector<pair<pii, int>>> v(n);
    FOR(i, 0, n - 1) 
    {
        int m; cin >> m;
        a[i].resize(m);
    ...
    vi pos(n, 0), done(n, 0);
    FOR(i, 0, n - 1) if (!v[i].empty()) 
    {
        pq.push({v[i][0].fi.fi, i});
    }
 
    while (!pq.empty() && pq.top().fi <= x) 
    {
        ...
        pos[id]++;
        if (pos[id] < (int)v[id].size()) pq.push({v[id][pos[id]].fi.fi, id});
    }
 
    int mx = -1, ans = 1;
    FOR(i, 0, n - 1) 
