...
Date: 2026-08-17 21:36:09
Problem name: C. Spying on the Beaver
Problem URL: https://codeforces.com/contest/2257/problem/C
Idea: 
Tag used: 
Summary: 
*/
...
void solve()
{
    ...
    {
        vi Z;
        EACH(v, g[u]) if (hasZero[v]) Z.pb(v);
        int K = (int)Z.size() + (damnIT[u] ? 1 : 0);
        int cameraMan = max(0LL, K - 1);
        for (int i = 0; i < cameraMan; i++)  res.pb(Z[i]);
        hasZero[u] = (K > 0);
    }
    cout << res.size();
    EACH(x, res) cout << " " << x;
