...
Date: 2026-08-17 21:53:51
Problem name: D. Bermuda Rectangle
Problem URL: https://codeforces.com/contest/2257/problem/D
Idea: 
Tag used: 
Summary: 
*/
...
constexpr int ALPHA = 26, BIT = 30, MAXV = 50000000;
...
void solve()
{
    ...
    FOR(i, 0, m - 1) 
    {
        W[i] = div[i];
        H[i] = S / div[i];
        int pW = (i == 0) ? 0 : div[i - 1];
        len[i] = div[i] - pW;
    }
    vi prefH(m + 1, 0), prefLen(m + 1, 0);
    FOR(i, 0, m - 1)
    {
        prefH[i + 1] = prefH[i] + len[i] * H[i];
    ...
    while(q--)
    {
        ...
        minimize(x, S);
        int kk = lower_bound(nall(div), x) - div.begin();
        int cut = lower_bound(nall(div), (S + y - 1) / y) - div.begin();
        minimize(cut, kk);
        int res = prefLen[cut] * y;
        if(cut < kk) res += (prefH[kk] - prefH[cut]);
        int prevW = (kk == 0) ? 0 : div[kk - 1];
        int remLen = x - prevW;
        if(remLen > 0) res += remLen * min(y, H[kk]);
        cout<<res<<endl;
    }
}
