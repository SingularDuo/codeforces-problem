...
Date: 2026-08-08 10:12:07
Problem name: F. Make It One
Problem URL: [https://codeforces.com/problemset/problem/1043/F](https://codeforces.com/problemset/problem/1043/F)
Idea: 
chọn tập ít phần tử nhất sao cho gcd = 1
phân tích thừa số nt (6 số)
Tag used: 
Summary: 
*/
#include <cassert> 
...
 
int n;
int a[MAXN + 5], fact[MAXN + 5], invFact[MAXN + 5], g[MAXN + 5];
int c[MAXN + 5];
 
ll binpow(ll a, ll b) {
    ll res = 1;
...
ll modInverse(int n){return binpow(n, MOD - 2);}
void precompute(int limit = 3e5)
{
    fact[0] = 1;
    invFact[0] = 1;
