...
int n, m;
int cost[MAXN + 5], fCost[MAXN + 5];
int dp[MAXN + 5];
int st[5 * MAXN + 5], lazy[5 * MAXN + 5];
void push(int id)
{
    if(lazy[id] != 0)
    {
...
int get(int id, int l, int r, int ql, int qr)
{
    ...
    push(id);
    int mid = (l + r) / 2;
    return max(get(id * 2, l, mid, ql, qr), get(id * 2 + 1, mid + 1, r, ql, qr));
}
 
_______TOISECOGIAIVOI_______
{
    ...
    FOR(i, 1, n) fCost[i] = fCost[i - 1] + cost[i];
    FOR(i, 1, m) cin>>race[i].l>>race[i].r>>race[i].p;
    sort(race + 1, race + m + 1);
    int raceID = 1;
    FOR(i, 1, n) 
    {
        dp[i] = dp[i - 1];
        ...
        //     {
        //         if (race[k].r > i) break; 
        //         if (race[k].l >= j && race[k].r <= i) totalP += race[k].p;
        //     }
        //     int currVal = (dp[j - 1] + fCost[j - 1] + totalP) - fCost[i];
        //     maximize(dp[i], currVal);
        // }
        while(raceID <= m && race[raceID].r == i)
        {
            int l = race[raceID].l;
            update(1, 1, n, 1, l, race[raceID].p);
            raceID++;
        }
        maximize(dp[i], get(1, 1, n, 1, i) - fCost[i]);
        if(i < n) update(1, 1, n, i + 1, i + 1, dp[i] + fCost[i]);
    }
    cout << dp[n];
 
    TIMER(start);
