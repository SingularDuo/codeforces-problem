...
pii st[1200005]; 
vi coord;
void update(int id, int l, int r, int pos, pii curr) 
{
    ...
    int mid = (l + r) / 2;
    update(id * 2, l, mid, pos, curr);
    update(id * 2 + 1, mid + 1, r, pos, curr);
    st[id] = max(st[id * 2], st[id * 2 + 1]);
}
 
pii get(int id, int l, int r, int u, int v) 
{
    if (v < l || u > r || u > v) return {0, 0};
...
_______TOISECOGIAIVOI_______
{
    ...
    for (int i = 1; i <= n; i++) 
    {
        ...
        int R = getID(h[i] + d);
        int curr = getID(h[i]);
        pii resL = get(1, 1, m, 1, L);
        pii resR = get(1, 1, m, R, m);
        pii best = max(resL, resR);
        dp[i] = best.fi + 1;
        trace[i] = best.se;
        if (maximize(res, dp[i])) bestID = i;
        update(1, 1, m, curr, {dp[i], i});
    }
 
    cout << res << endl;
    vi path;
