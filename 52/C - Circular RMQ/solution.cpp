...
struct SegmentTree
{
    ...
    void update(int ql, int qr, int val, int id = 1, int l = 1, int r = n)
    {
        if(l > qr || r < ql) return;
        if(l >= ql && r <= qr) 
        {
        ...
        push(id);
        int mid = (l + r ) / 2;
        update(ql,qr,val,id * 2, l, mid);
        update(ql, qr, val, id * 2 + 1, mid + 1, r);
        st[id] = min(st[id * 2], st[id * 2 + 1]);
    }
    int get(int ql, int qr, int id = 1, int l = 1, int r = n)
    {
        if(l > qr || r < ql) return OO;
        if(l >= ql && r <= qr) return st[id];
        push(id);
        int mid = (l + r) / 2;
        return min(get(ql, qr, id * 2, l, mid), get(ql, qr, id * 2 + 1, mid + 1, r));
    }
}st;
 
 
_______TOISECOGIAIVOI_______
{
    ...
    while(q--)
    {
        ...
        {
            if (l <= r)  cout << st.get(l, r) << endl;
            else cout << min(st.get(l, n), st.get(1, r)) << endl;
        }
    }
 
