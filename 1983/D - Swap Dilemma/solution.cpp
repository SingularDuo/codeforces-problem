...
struct SegmentTree {
    ...
    int query(int id, int l, int r, int u, int v) {
        ...
        int mid = (l + r) / 2;
        int sub_left = query(2 * id, l, mid, u, v);
        int sub_right = query(2 * id + 1, mid + 1, r, u, v);
...
void solve()
{
    ...
    for(int i = 1; i <= n; i++) 
    {
        a[i] = lower_bound(nall(coord), a[i]) - coord.begin() + 1;
        iA[a[i]] = i;
        b[i] = lower_bound(nall(coord), b[i]) - coord.begin() + 1;
        iB[b[i]] = i;
    }
    vi A, B;
    for(int i = 1; i <= n; i++)
    ...
    for(int i = n; i >= 1; i--)
    {
        res1 += st1.query(1, 1, n, 1, a[i] - 1);
        st1.update(1, 1, n, a[i], 1);
        res2 += st2.query(1, 1, n, 1, b[i] - 1);
        st2.update(1, 1, n, b[i], 1);
    }
    if((res1 % 2) == (res2 % 2)) 
    {
        cout<<"Yes"<<endl;
