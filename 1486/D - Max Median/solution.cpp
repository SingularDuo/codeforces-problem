...
int n, k;
int a[MAXN + 5], f[MAXN + 5], temp[MAXN + 5];
bool check(int x)
{
    for (int i = 1; i <= n; i++) temp[i] = (a[i] >= x) ? 1 : -1;
    f[0] = 0;
    for (int i = 1; i <= n; i++) f[i] = f[i - 1] + temp[i];
    int minPrefix = OO;
    for (int i = k; i <= n; i++) 
    {
        minPrefix = min(minPrefix, f[i - k]);
        if (f[i] > minPrefix) return true;
    }
    return false;
}
